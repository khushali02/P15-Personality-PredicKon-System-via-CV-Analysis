...
def analyze_resume():
    try:
        ...
        if file:
            ...
            name = extract_name(text, row_number)  # Corrected parameter
            ...
            query = "Describe a candidate's personality if their traits are: " + ', '.join(
                f"{k}: {v}" for k, v in personality_traits.items())
            response = chat(query)

            # Render the result page with extracted details and AI response
